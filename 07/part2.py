def can_equate(goal_total: int, partial_total: int, nums: list[int], start_index: int) -> bool:
    if start_index == len(nums):
        ...
        return False
    return can_equate(goal_total, partial_total + nums[start_index], nums, start_index + 1) or can_equate(goal_total, partial_total * nums[start_index], nums, start_index + 1) or can_equate(goal_total, int(str(partial_total) + str(nums[start_index])), nums, start_index + 1)
